...
static async createExportDetails(maPhieuXuat, maHangHoa, soLuong, donGia) {
  return new Promise((resolve, reject) => {
      const query = `CALL CreateChiTietPhieuXuatHang(${db.escape(maPhieuXuat)}, ${db.escape(maHangHoa)}, ${db.escape(soLuong)}, ${db.escape(donGia)})`;
      db.query(query, (err, results) => {
          if (err) {
              return reject(err);
          }
...
static getProductPrice(maHangHoa) {
  return new Promise((resolve, reject) => {
    const query = `SELECT don_gia FROM HangHoa WHERE ma_hang_hoa = ${db.escape(maHangHoa)}`;
    db.query(query, (err, results) => {
      if (err) return reject(err);
      resolve(results);
    });
...
static updateTotalAmount(maPhieuXuat, totalAmount) {
  return new Promise((resolve, reject) => {
    const query = `UPDATE PhieuXuatHang SET tong_tien = ${db.escape(totalAmount)} WHERE ma_phieu_xuat = ${db.escape(maPhieuXuat)}`;
    db.query(query, (err, results) => {
      if (err) return reject(err);
      resolve(results);
    });
