...
exportTable.addEventListener("input", (event) => {
    if (event.target.classList.contains("quantity")) {
        ...
        const validRows = Array.from(exportTable.querySelectorAll("tr")).filter(row => {
            const selectedOption = row.querySelector(".item-select")?.value;
            return selectedOption;
        });
...
//     // Lấy danh sách mặt hàng từ bảng
//     const danhSachMatHang = Array.from(document.querySelectorAll("#export-table tbody tr"))
//     .filter(row => row.querySelector(".item-select").value)
//     .map(row => {
//         const maHangHoa = row.querySelector(".item-select").value;
//         const soLuong = parseInt(row.querySelector(".quantity").value) || 0;
...
//             method: "POST",
//             headers: { "Content-Type": "application/json" },
//             body: JSON.stringify({ maDaiLy, ngayLapPhieu, danhSachMatHang }),
//         });

//         const result = await response.json();
//         if (response.ok) {
//             alert(result.message);
//             // Reset hoặc làm mới giao diện
//         } else {
...
document.getElementById("export-btn").addEventListener("click", async () => {
    ...
    try {
      const response = await fetch("http://localhost:3000/api/create-export", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({maDaiLy,ngayLapPhieu,Products: danhSachMatHang }),
      });
